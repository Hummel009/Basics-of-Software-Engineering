...
public class Lab1_06_2 {
	...
	public static void main(String[] args) {
		...
		int[] date = Arrays.stream(in.nextLine().replaceAll("\\s", "").split("/")).mapToInt(Integer::parseInt).toArray();
		if (date[2] < 1) {
		...
		if (date[1] > 1) {
			diiy += Months.JAN.getDays();
		}
		if (date[1] > 2) {
			diiy += Months.FEB.getDays() - 1;
		}
		if (date[1] > 3) {
			diiy += Months.MAR.getDays();
		}
		if (date[1] > 4) {
			diiy += Months.APR.getDays();
		}
		if (date[1] > 5) {
			diiy += Months.MAY.getDays();
		}
		if (date[1] > 6) {
			diiy += Months.JUN.getDays();
		}
		if (date[1] > 7) {
			diiy += Months.JUL.getDays();
		}
		if (date[1] > 8) {
			diiy += Months.AUG.getDays();
		}
		if (date[1] > 9) {
			diiy += Months.SEP.getDays();
		}
		if (date[1] > 10) {
			diiy += Months.OCT.getDays();
		}
		if (date[1] > 11) {
			diiy += Months.NOV.getDays();
		}
